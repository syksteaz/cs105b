...
class MovieData
	...
	def load_training(filepath, filename)
	...
	def most_similar(user)
		return @users
		.map{|other_user_id, other_user| [other_user, similarity(user, other_user)]}
	...
	def run_test(s)
		s ||= @users_test.length
		subset = @users_test.first s
		return MovieTest.new(subset
		.map{|id, user| 
			user.get_movies
       			.map{|movie, rating|
			[
				id,
	 			movie.get_id,
				rating,
				predict(retrieve_user(user), retrieve_movie(movie))
			]
			}
		}).mean
	end
	class MovieTest
		...
		def mean
			return @data.map{|user_id, movie_id, rating, prediction|
				puts movie_id
				(prediction.to_f - rating.to_f)}.reduce(:+).to_f / length.to_f
		end
		def stdev
		...
		def variance
			mean_value = mean
			return @data.map{|user_id, movie_id, rating, prediction| (prediction - rating - mean_value)**2}.reduce(:+).to_f / length.to_f
		end
		def rms
			return sqrt(mean)
		end
...
if ARGV.empty?
	...
	begin
		...
		when '-run_test'
			puts mv.run_test(ARGV.shift.to_i)
		else
